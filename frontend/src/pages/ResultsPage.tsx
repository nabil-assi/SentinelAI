          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              ...
              {vuln.cveId && vuln.cveId !== "N/A" && (
                ...
                >
                  {vuln.cveId} <ExternalLink className="h-2 w-2" />
                </a>
              )}
            </div>
            <span className="text-xs text-muted-foreground truncate max-w-[400px] mt-1 italic">
              {vuln.title || `${vuln.libraryName} vulnerability`}
            </span>
            ...
            <div className="p-6 border-t border-border/50 space-y-5">
              ...
              <div className="space-y-3">
                ...
                <pre className="p-5 rounded-xl bg-black text-emerald-400 text-[11px] font-mono overflow-x-auto border border-white/5">
                  <code>
                    {vuln.recommendation ||
                      "Check for updates or patches for this library."}
                  </code>
                </pre>
              </div>
            </div>
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const res = await api.get(`/api/scan/results/${scanId}`);
        console.log("Full response:", res);
        console.log("Response data:", res.data);

        // ✅ جلب البيانات من res.data.data
  ...
  // جلب الثغرات من aiInsights.prioritizedVulnerabilities
  const vulns = scan.aiInsights?.prioritizedVulnerabilities || [];
              ...
              <div className="p-1 space-y-1">
                ...
                <button
                  onClick={() => {
                    ...
                    linkElement.setAttribute(
                      "download",
                      `vulnerabilities-${scan.id.slice(0, 8)}.csv`,
                    );
                    linkElement.click();
                  }}
                  className="w-full text-left px-4 py-2 text-sm hover:bg-primary/10 rounded-lg transition-colors"
          ...
          return (
            <div key={severity} className="space-y-4">
              ...
              <div className="grid gap-3">
                {items.map((v: any) => (
                  <VulnCard key={v.cveId || v.libraryName} vuln={v} />
                ))}
              </div>
            </div>
          );
